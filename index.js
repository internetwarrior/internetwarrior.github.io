...
//resource
const THE_QUESTION_MARK_LINK = "https://www.youtube.com/watch?v=2Ep6VmdKjGE&list=RD2Ep6VmdKjGE&start_radio=1"
...
document.querySelector(".hint").innerText = hints[Math.floor(Math.random() * hints.length)]
...
function processArrayBuffer(arrayBuffer) {
	...
	function fadeToZero() {
		...
		gainNode.gain.linearRampToValueAtTime(backgroudVolume, currentTime + fadeDuration)
	}
	...
	function restoreVolume() {
		const currentTime = audioContext.currentTime
		gainNode.gain.setValueAtTime(gainNode.gain.value, currentTime)
		gainNode.gain.linearRampToValueAtTime(originalVolume, currentTime + fadeDuration)
	}
...
checkbox.addEventListener("change", () => {
	antiHero.style.animation = checkbox.checked ? "floating-2 4000ms infinite ease alternate" : "none"
...
async function loadDefaultAudio() {
	...
	swear.style.animation = "borderDisappear var( --animation-duration) forwards ease-out"
...
function visualize(audioBuffer, audioContext, gainNode) {
	...
	function draw() {
		...
		for (let i = 0; i < frequencyBufferLength; i++) {
			...
			const color = "rgb(" + (COLOR_OBJ.color_1 + value) + `,${COLOR_OBJ.color_2}, ${COLOR_OBJ.color_3})`

			canvasContext.fillStyle = color

			const rightX = midX + i * barWidth - canvas.width / 2
			canvasContext.fillRect(rightX, canvas.height - value / 1.2, barWidth - BAR_WIDTH, value)

			const leftX = midX_2 - i * barWidth
			canvasContext.fillRect(leftX, canvas.height - value / 1.2, barWidth - BAR_WIDTH, value)
		}
	}
...
el.addEventListener("mouseleave", (event) => {
	clearInterval(interval)
	event.target.innerText = WORD_STORAGE[Math.floor(Math.random() * WORD_STORAGE.length)] // вернуть нормальный текст
})
